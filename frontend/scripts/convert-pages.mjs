...
for (const [src, dst] of Object.entries(map)) {
  ...
  if (dst.includes("SaleReceipt")) {
    c = 'import { Link, useParams } from "react-router-dom";\n' + c.replace(/import \{ Link \} from "react-router-dom";\r?\n/, "");
    c = c.replace(
  ...
  if (dst.includes("Employees")) {
    ...
    c = c.replace(/navigate\("\/dashboard", \{ replace: true \}\)/g, 'navigate("/dashboard", { replace: true })');
  }
  ...
  if (dst.includes("NewSale")) {
    ...
    if (!c.includes('import { Link')) {
      c = 'import { Link, useNavigate } from "react-router-dom";\n' + c;
    }
  }
